...
export async function LinkBuildingPanel() {
  ...
  // `accent` colors only the ICON (graphics need 3:1, these pass); the label
  // text inherits the theme text color so it stays AA-readable in both themes.
  const CARDS = [
    { icon: <Trophy size={18} />, label: 'Links won this month', value: String(wonThisMonth), sub: `${backlinks.length} total · ${liveLinks} live · ${lostLinks} lost`, href: '/admin/collections/backlinks', accent: '#22c55e' },
    { icon: <Clock size={18} />, label: 'Pending follow-ups', value: String(followUps.length), sub: followUps.length ? 'due now — see below' : 'nothing due', href: '/admin/collections/link-prospects', accent: '#f59e0b' },
    { icon: <Link2 size={18} />, label: 'Outreach win rate', value: winRate === null ? '—' : `${Math.round(winRate * 100)}%`, sub: `${won} won · ${rejected} rejected`, href: '/admin/collections/link-prospects', accent: '#3b82f6' },
    { icon: <Globe size={18} />, label: 'Sites embedding tools', value: String(byHost.size), sub: `${embeds.reduce((s, e) => s + (e.count || 1), 0)} embed loads`, href: '/admin/collections/embed-logs', accent: '#8b5cf6' },
  ];
        ...
        {CARDS.map((c) => (
          <a key={c.label} href={c.href} style={{ textDecoration: 'none', color: 'inherit', border: '1px solid var(--theme-elevation-100)', borderRadius: 10, padding: '0.75rem' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700 }}>
              <span style={{ color: c.accent, display: 'inline-flex' }} aria-hidden="true">{c.icon}</span> {c.label}
            </span>
