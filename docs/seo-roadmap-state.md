...
- **Current phase:** 14 — HOTFIX 1+2 complete and PROVEN LIVE; P1–P3 ready to start
- **Off-phase technical SEO fix (2026-07-18, user-driven audit):** `/tools` HTML payload 197 KB → **153 KB** (−22%): Icon.astro now emits `<use>` refs to a generated sitewide sprite (`scripts/gen-icon-sprite.ts` → `public/icons/sprite.svg`, wired as `prebuild`; 59 icons / 12 KB, cached cross-page); lucide presentation attrs collapsed into one `.icn` CSS rule (~150 B/icon); ToolIcon gradient palette moved to CSS classes (inline style only for custom sizes); `/tools` search drops duplicated `data-desc` (reads blurb from DOM). **Orphan tag pages rescued:** `heart-health` + `mental-wellness` had zero articles AND zero inbound links (tag pages generated for every CMS tag; only inbound source was article tag chips; orphan-check ignored `/wellness-hub/tag/*`). Fixed via CMS REST (auth note: `.env` password values are quote-wrapped — strip quotes before login): attached both tags to 7 relevant articles + appended contextual in-body links in 6 of them (article 23 `how-to-keep-a-food-and-symptom-diary` PATCHes → 500 on ANY payload, doc-specific server-side issue — **needs manual admin re-save**). Structural: `/wellness-hub` gained a "Browse by subject" hub section linking every live tag page; `tag/[slug]` no longer generates zero-article tag pages; `orphan-check.ts` now TRACKS tag pages (114 tracked, 0 orphans, 0 dead links; JSON-LD + CWV audits green).
- **Hotfix 2 summary (2026-07-10):** spec's premise falsified by live audit (URLs absolute, not relative; scan errors never persisted — panel state rebuilds per session). REAL root cause: `content-urls` derived its origin from `NEXT_PUBLIC_SITE_URL` (= APEX domain on prod, no www.) → all 90 scan URLs on the wrong origin. Fixed: `content-urls` now uses validated `SITE_BASE_URL`; new pure `toAbsoluteInspectionUrl` boundary (9 vitest fixtures) heals relative + apex→www, throws typed LocalValidationError on foreign origins; `check-indexing` results carry taxonomy `{source: LOCAL_VALIDATION|GOOGLE_API, checkedAt}` with "Skipped locally:"/"Google API:" prefixes. **LIVE GATE: 90/90 fresh scans, ZERO errors, zero Permission denied — 57 "Submitted and indexed", 28 "Discovered - currently not indexed", 5 "URL is unknown to Google"; cms.-origin fixture refused locally; relative fixture normalized + genuinely inspected.**
- **Previous hotfix:** HOTFIX 1 (GSC Permission-denied — siteUrl validation + credential chain + debug endpoint)
- **Hotfix summary (2026-07-10):** root cause = hypothesis B: `check-indexing` read raw `GSC_SITE_URL || NEXT_PUBLIC_SITE_URL` (slash-less fallback can never equal a URL-prefix property), bypassing `site-config.ts` validation. Hypotheses A/C falsified (loader was already fail-loud B64-only; errors were authenticated 403s). Fix: route now uses the module-load-validated `GSC_SITE_URL` (both property forms), rejects URLs outside the property, inspects sequentially with 429 backoff, structured logging; loader accepts plain-JSON→plain-b64→B64 (still fail-loud, never unauthenticated) with client_email drift warning; new flag-gated (`SEO_DEBUG_ENDPOINTS=true`) admin-only `/api/seo/test-credentials` self-diagnosis endpoint (sites.list + live inspect). **PROVEN LIVE post-deploy:** homepage inspection through the Scan-All path → "Submitted and indexed", error null — zero Permission denied. Remaining human steps: set flag → endpoint round-trip → Scan All → unset flag.
- **Previous phase:** 13 — Breadcrumb Strict Compliance · Baseline-Aware Audit Gate (frontend repo)
- **Phase 13 summary (2026-07-10):** P1: all 7 BreadcrumbList emitters now emit `item` as `{"@id": <canonical URL>, "name"}` entity references (last-item omission/positions/names preserved; tag-page trailing-slash inconsistency normalized; shape proven on the validator BEFORE editing) — Phase 12's 11 informational findings → **0 errors/0 warnings across all 7 page types**, CI stub 5/5 clean. P2: `audit-baseline.json` (22 legacy vulns = accepted-risk ledger, 9 advisory ids, reviewBy 2026-10-08) + dependency-free `scripts/ci/audit-delta.ts` wired into promote.yml — fails ONLY on NEW high/critical, warn-only moderate/low, hard-fails on baseline expiry; regression-tested (lodash@4.17.15 → 3 new HIGH → exit 1) with pristine tree. Divergence: npm audit was previously enforced nowhere — gate ADDED, not replaced.
- **Previous phase:** 12 — Sandboxed SHACL Validation (frontend repo)
